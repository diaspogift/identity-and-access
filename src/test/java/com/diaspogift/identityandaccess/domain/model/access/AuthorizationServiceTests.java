...
public class AuthorizationServiceTests extends IdentityAndAccessTest {
    ...
    @Test
    public void allRolesForIdentifiedUse() throws Exception {
        ...
        Role infirmierChefRole = tenant.provisionRole(FIXTURE_ROLE_NAME_1, FIXTURE_ROLE_DESCRIPTION_1, true);
        medecinChefRole.assignUser(user);
        infirmierChefRole.assignUser(user);
        ...
        Collection<RoleDescriptor> allRolesForUser = DomainRegistry.authorizationService().allRolesForIdentifiedUser(tenant.tenantId(), user.username());


        assertNotNull(allRolesForUser);
        assertEquals(2, allRolesForUser.size());

        allRolesForUser.stream().forEach(roleDescriptor -> System.out.println("roleDescriptor = " + roleDescriptor));
